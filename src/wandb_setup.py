...
def setup_hyper_opt(main, nn_config):
    ...
    if nn_config['opt_method'] == 'twolosses':
        ...
        sweep_config = {
            ...
            'parameters': {
                ...
                'layers': {'values': [4]},
                'loss': {'values': ['focalLoss', 'CrossEntropyLoss', 'non_weighted_CrossEntropyLoss', 'NLLLoss']},
                'focal_loss_scale': {'values': [1.5]},  # optimal 2.3
        ...
        sweep_config = {
            'method': 'grid',
            'name': f"exp_s_{nn_config['sample_size']}_l_{nn_config['seq_length']}_sn_{nn_config['sn_ratio']}c",
            'metric': {'goal': 'maximize', 'name': 'f1_val'},
            'parameters': {
                'learning_rate': {'values': [0.002]},
                'batch_size': {'values': [32, 64]},
    ...
    wandb.agent(sweep_id, function=main, count=400, project="train-classsifier")
...
def cnn_hyperparameters(wandb_active, hyperparam_opt, nn_config, config_file):
    ...
    if wandb_active:
        if hyperparam_opt:
            ...
            with open('src/regressor.yaml', 'w') as file:
