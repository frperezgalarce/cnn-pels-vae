...
from scipy.stats import norm  # For inverse CDF of Gaussian
...
class VAE_TCN(nn.Module):
    ...
    def __init__(self, latent_dim, seq_len, kernel_size,
        ...
        #self.R = nn.Parameter(torch.eye(latent_dim))  # Correlation matrix, initialized to identity
...
False
    ...
    def forward(self, x, label=None, phy=None):
        ...
        if copulaMVG:
                # Copula modeling using Gaussian copula (parameterized by correlation matrix R)
                self.R = nn.Parameter(self.R + torch.eye(self.R.size(0), device=self.R.device) * 1e-6)
                copula_dist = MultivariateNormal(torch.zeros(self.latent_dim).to(device), self.R.to(device))
                u = torch.sigmoid(z)
                u = u.to(device)  # Assuming `device` is defined earlier and contains either "cpu" or "cuda"
                copula_loss = -copula_dist.log_prob(u)  # Negative log-likelihood
                xhat = self.decoder(u, x[:,:,0], label=label, phy=phy)
                torch.save({
                    'mean': torch.zeros(self.latent_dim).to(device),
                    'covariance_matrix': self.R
                }, PATH_MODELS+'copula_model.pth')

                return xhat, mu, logvar, z, copula_loss
        else:
            xhat = self.decoder(z, x[:,:,0], label=label, phy=phy)
            return xhat, mu, logvar, z, None
