...
def setup_hyper_opt(main, nn_config): 
    ...
    if nn_config['training']['opt_method'] == 'twolosses':
        ...
        sweep_config = {
            'method': 'bayes',
            'name': f"exp_s_{nn_config['data']['sample_size']}_l_{nn_config['data']['seq_length']}_sn_{nn_config['data']['sn_ratio']}- hyperparameter search 0.3",
            'metric': {'goal': 'maximize', 'name': 'weighted_f1'},
            'parameters': {
                'learning_rate': {'min': 0.02, 'max':0.1},
                'batch_size': {'values': [32, 64, 128]},
                'patience': {'min': 5, 'max':20},
                'repetitions': {'min': 1, 'max':16},
                'synthetic_samples_by_class':  {'min': 8, 'max':32},  
                'threshold_acc_synthetic': {'min': 0.5, 'max':0.9},
                'beta_decay_factor': {'values': [1]},
                'EPS': {'values': [0.15, 0.2, 0.25, 0.3, 0.35]},  
                'scaling_factor': {'min': 0.5, 'max':2.0},
                'vae_model': {'values': ['gn42liaz']},
                'sufix_path': {'values': ['GAIA3_LOG_IMPUTED_BY_CLASS_6PP','GAIA3_LOG_6PP']},
                'layers': {'values': [2, 3, 4]},
                'loss': {'values': ['focalLoss']}, 
                'focal_loss_scale': {'min': 1, 'max':4},  
                'n_oversampling': {'values': [2, 4, 8, 16]},
                'decay_parameter_1': {'min': 0.5, 'max':0.8},
                'decay_parameter_2': {'min': 0.1, 'max':0.9},
                'ranking_method': {'values': ['proportion', 'no_priority', 'CCR', 
                                              'max_confusion', 'max_pairwise_confusion']},
            }
        }
