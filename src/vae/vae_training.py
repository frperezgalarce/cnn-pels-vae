...
class Trainer():
    ...
    def _loss(self, x, xhat, mu, logvar, train=True, ep=0, copula_loss=None):
        ...
        kld_l = -0.5 * torch.sum(1 + logvar - mu.pow(2) -
                                 logvar.exp()) / x.shape[0] / 1e4
        # kld_o = -1. * F.kl_div(xhat[:,:,-1], x[:,:,-1], reduction='mean')
        # use MSE converge faster and better
       # Ensure copula_loss is a scalar
        if copula_loss is not None:
            if len(copula_loss.shape) > 0:
                copula_loss = 0.001*copula_loss.sum()

        # Ensure mse and kld_l are scalars (for debugging)
        assert len(mse.shape) == 0 and len(kld_l.shape) == 0, f"MSE and KLD_L must be scalars. Got shapes {mse.shape}, {kld_l.shape}"
    ...
    def _train_epoch(self, data_loader, epoch):
        ...
        for i, (data, label, onehot, pp) in enumerate(data_loader):
            ...
            if self.cond_l and not self.cond_p:
                xhat, mu, logvar, z, copula_loss = self.model(data, label=onehot)
            elif self.cond_p and not self.cond_l:
                xhat, mu, logvar, z, copula_loss = self.model(data, phy=pp)
            elif self.cond_l and self.cond_p:
                xhat, mu, logvar, z, copula_loss = self.model(data, label=onehot, phy=pp)
            else:
                xhat, mu, logvar, z, copula_loss = self.model(data)

            loss = self._loss(data[:, :, 1:], xhat,
                              mu, logvar, train=True, ep=epoch, copula_loss=copula_loss)
            loss.backward()
            self.opt.step()
        ...
        with torch.no_grad():
            ...
            for i, (data, label, onehot, pp) in enumerate(test_loader):
                ...
                if self.cond_l and not self.cond_p:
                    ...
                    xhat, mu, logvar, z, copula_loss = self.model(
                        data, label=onehot, phy=pp)
                else:
                    xhat, mu, logvar, z, copula_loss = self.model(data)

                loss = self._loss(data[:, :, 1:], xhat,
                                  mu, logvar, train=False, ep=epoch, copula_loss=copula_loss)
                # concat time to recon
                xhat = torch.cat([data[:, :, 0].unsqueeze(-1), xhat], dim=-1)
